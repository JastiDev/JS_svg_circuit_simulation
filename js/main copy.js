...
let arrColor = ['yellow', 'fuchsia', 'lime', 'aqua', 'maroon', 'blue', 'olive', 'purple', 'green', 'teal'];
let pointColor = -1;
    ...
    if (evt.keyCode == 32) {
        ...
        if (KA != -1 && KB != -1) {
            if (!conn[KA][KB]) {
                conn[KA][KB] = true;
                conn[KB][KA] = true;
                console.log(arrP[KA].used);
                if (arrP[KA].used == 0 && arrP[KB].used == 0) {
                    pointColor++;
                    arrP[KA].used = pointColor;
                    arrP[KB].used = pointColor;
                }
...
function connColoring() {
    ...
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            ...
            if (arrP[k].used > 0) {
                console.log(k);
                point.setAttributeNS(null, "stroke", arrColor[arrP[k].used - 1]);
            }
