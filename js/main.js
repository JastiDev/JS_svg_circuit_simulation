  ...
  if (evt.keyCode == 32) {
    ...
    if (KA != -1 && KB != -1) {
      if (!conn[KA][KB]) {
        ...
        let ele = parseSVG(`<line id="line_${Math.min(KA, KB)}_${Math.max(
          KA,
          KB
        )}" 
  ...
  if (evt.shiftKey) {
    ...
    for (let l = 0; l < conn[KMA].length; l++) {
      if (conn[KMA][l] == true) {
        ...
        let ele = parseSVG(`<line id="line_${Math.min(KMB, l)}_${Math.max(
          KMB,
          l
        )}" 
                                    x1="${arrP[KMB].cx}" y1="${arrP[KMB].cy}" 
                                    x2="${arrP[l].cx}" y2="${arrP[l].cy}"
                                    stroke=${used_color} style="pointer-events:none;"/>`);
...
function toggleUnused(_isShowUnused) {
  ...
  document.getElementById("btn_show_unused").style.display = isShowUnused
    ? "none"
    : "block";
  document.getElementById("btn_hide_unused").style.display = isShowUnused
    ? "block"
    : "none";
